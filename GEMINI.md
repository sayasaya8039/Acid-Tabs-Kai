...
- **Cursor** / **Windsurf** / **Anyigravity** その他AIコーディングアシスタント
...
- **このGEMINI.mdのルールを必ず遵守すること**
- **GEMINI.md読み込み時に `.gemini/` 配下の全ファイルも必ず読み込むこと**
  - `.gemini/settings.json` - 権限設定
  - `.gemini/rules/*.md` - 全ルールファイル（並列読み込み推奨）
...
- **API機能実装時は必ず最新モデルを使用すること**
  ...
  - 古いモデル名（例: gpt-3.5-turbo、GEMINI-2）は使用禁止
...
| 2025年12月18日 | .gemini/配下ファイルの読み込み指示を強化 |

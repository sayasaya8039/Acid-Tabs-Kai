...
(()=>{
  ...
  async function restoreTabOrder() {
    ...
    try {
      ...
      for (const win of windows) {
        ...
        const currentTabByUrl = new Map();

        for (const tab of currentTabs) {
          const id = getTabIdentifier(tab);
          if (!currentTabMap.has(id)) {
            currentTabMap.set(id, tab);
          }
          if (!currentTabByUrl.has(tab.url)) {
            currentTabByUrl.set(tab.url, tab);
          }
        }
        ...
        for (let i = 0; i < savedData.tabs.length; i++) {
          const savedTab = savedData.tabs[i];
          if (savedTab.pinned) continue;

          let matchingTab = currentTabMap.get(savedTab.id);
          if (!matchingTab) {
            matchingTab = currentTabByUrl.get(savedTab.url);
          }

          if (matchingTab && !processedTabs.has(matchingTab.id)) {
            processedTabs.add(matchingTab.id);

            if (savedTab.groupInfo && savedTab.groupInfo.title) {
              const groupKey = savedTab.groupInfo.title + '|' + savedTab.groupInfo.color;

              if (!newGroupMap.has(groupKey)) {
                try {
                  const newGroupId = await chrome.tabs.group({
                    tabIds: [matchingTab.id],
                    createProperties: { windowId: win.id }
                  });

                  await chrome.tabGroups.update(newGroupId, {
                    title: savedTab.groupInfo.title,
                    color: savedTab.groupInfo.color,
                    collapsed: false
                  });

                  newGroupMap.set(groupKey, newGroupId);
                } catch (e) {
                ...
                try {
                  await chrome.tabs.group({
                    tabIds: [matchingTab.id],
                    groupId: newGroupMap.get(groupKey)
                  });
                } catch (e) {}
              }
            }
        ...
        const updatedTabs = await chrome.tabs.query({ windowId: win.id });
        processedTabs = new Set();

        // 非ピン留めタブのみの順序リストを作成
        const savedUnpinnedTabs = savedData.tabs.filter(t => !t.pinned);

        for (let i = 0; i < savedUnpinnedTabs.length; i++) {
          const savedTab = savedUnpinnedTabs[i];

          let matchingTab = updatedTabs.find(t =>
            !t.pinned && (getTabIdentifier(t) === savedTab.id || t.url === savedTab.url)
          );
        ...
        // グループの折りたたみ状態を復元
        for (const [groupKey, groupId] of newGroupMap) {
          const parts = groupKey.split('|');
          const title = parts[0];
          const color = parts[1];
          const savedGroup = savedData.groups && savedData.groups.find(g => g.title === title && g.color === color);
  ...
  async function handleStartup() {
    console.log('[タブ順序] ブラウザ起動を検出');
    isRestoring = true;

    try {
      // 少し待ってからChromeの初期化を待つ
  ...
  console.log('[タブ順序] 管理機能 v2.1 を初期化しました');
})();

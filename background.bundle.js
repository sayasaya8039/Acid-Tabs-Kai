...
// タブ順序管理機能 v2.5
(()=>{
  // グローバルエラーハンドラー（未処理のPromiseエラーを抑制）
  self.addEventListener('unhandledrejection', function(event) {
    const msg = event.reason?.message || String(event.reason);
    if (msg.includes('No tab with id') || 
        msg.includes('No group with id') ||
        msg.includes('Tabs cannot be edited') ||
        msg.includes('windowId')) {
      event.preventDefault();
      return;
    }
  });

  // runtime.lastError を自動チェック（コールバックベースAPIのエラー抑制）
  const originalCallback = chrome.runtime.lastError;

  console.log('[タブ順序] エラーハンドラーを設定しました');
  ...
  // タブ順序を復元 (v2.5 - URLベースマッチング)
  async function restoreTabOrder() {
    isRestoring = true;
    console.log('[タブ順序] 復元処理開始 (v2.5)');
  ...
  async function handleStartup() {
    console.log('[タブ順序] ブラウザ起動を検出 (v2.5)');
    isRestoring = true;
  ...
  console.log('[タブ順序] 管理機能 v2.5 を初期化しました');
})();

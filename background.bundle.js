...
// タブ順序管理機能 v2.4
(()=>{
  // Chrome APIのエラーハンドリングをラップ（元のAcid Tabsコードのエラー抑制）
  const originalTabsGet = chrome.tabs.get.bind(chrome.tabs);
  chrome.tabs.get = function(tabId) {
    return originalTabsGet(tabId).catch(e => null);
  };

  const originalTabsUngroup = chrome.tabs.ungroup.bind(chrome.tabs);
  chrome.tabs.ungroup = function(tabIds) {
    return originalTabsUngroup(tabIds).catch(e => null);
  };

  const originalTabsMove = chrome.tabs.move.bind(chrome.tabs);
  chrome.tabs.move = function(tabIds, moveProperties) {
    return originalTabsMove(tabIds, moveProperties).catch(e => null);
  };

  const originalTabGroupsUpdate = chrome.tabGroups.update.bind(chrome.tabGroups);
  chrome.tabGroups.update = function(groupId, updateProperties) {
    return originalTabGroupsUpdate(groupId, updateProperties).catch(e => null);
  };

  const originalTabGroupsMove = chrome.tabGroups.move.bind(chrome.tabGroups);
  chrome.tabGroups.move = function(groupId, moveProperties) {
    return originalTabGroupsMove(groupId, moveProperties).catch(e => null);
  };

  console.log('[タブ順序] Chrome APIラッパーを適用しました');

  const TAB_ORDER_KEY = 'acidTabs:tabOrder';
    ...
    try {
      ...
      for (const win of windows) {
        ...
        const groupOrder = groups
          .map(g => {
            const firstTab = tabs.find(t => t.groupId === g.id);
            return {
              title: groupMap[g.id].title,
              color: groupMap[g.id].color,
              collapsed: groupMap[g.id].collapsed,
              firstIndex: firstTab ? firstTab.index : 999
            };
          })
          .sort((a, b) => a.firstIndex - b.firstIndex);

        orderData.windows[win.id] = {
          tabs: tabOrder,
          groups: groupOrder,
      ...
      console.log('[タブ順序] 保存完了:', new Date().toLocaleTimeString());
    } catch (e) {
      console.error('[タブ順序] 保存エラー:', e);
    }
  }

  // 即時保存
  async function saveTabOrderImmediately() {
    if (saveTimeout) {
  ...
  function debouncedSave() {
    ...
    saveTimeout = setTimeout(saveTabOrder, SAVE_DELAY);
  }

  // 全タブのグループを解除 (v2.4)
  async function ungroupAllTabs(windowId) {
    console.log('[タブ順序] 全タブのグループを解除中...');
    try {
      // 現在のタブを取得（最新の状態）
      ...
      console.log('[タブ順序] グループ解除完了');
    } catch (e) {
      console.error('[タブ順序] グループ解除エラー:', e);
    }
  }

  // タブ順序を復元 (v2.4 - URLベースマッチング)
  async function restoreTabOrder() {
    isRestoring = true;
    console.log('[タブ順序] 復元処理開始 (v2.4)');

    try {
      const result = await chrome.storage.local.get(TAB_ORDER_KEY);
      const orderData = result[TAB_ORDER_KEY];

      if (!orderData || !orderData.windows) {
        console.log('[タブ順序] 保存データなし');
        isRestoring = false;
        return;
      }

      if (Date.now() - orderData.timestamp > 24 * 60 * 60 * 1000) {
        console.log('[タブ順序] データが古いため復元をスキップ');
        isRestoring = false;
        return;
      }
      ...
      if (!savedData || !savedData.tabs || savedData.tabs.length === 0) {
        console.log('[タブ順序] 保存タブデータなし');
        isRestoring = false;
        return;
      }

      console.log('[タブ順序] 保存データ:', savedData.tabs.length, 'タブ,', (savedData.groups || []).length, 'グループ');

      for (const win of windows) {
        // ステップ1: 全タブのグループを解除
        await ungroupAllTabs(win.id);
        await new Promise(resolve => setTimeout(resolve, 500));

        // ステップ2: 現在のタブを取得（グループ解除後の最新状態）
        let currentTabs = await chrome.tabs.query({ windowId: win.id });
        const pinnedCount = currentTabs.filter(t => t.pinned).length;
        console.log('[タブ順序] 現在のタブ数:', currentTabs.length, '(ピン留め:', pinnedCount, ')');

        // URLでタブをグループ化（同じURLが複数ある場合に対応）
        ...
        // ステップ3: 保存順序でタブを並べ替え
        console.log('[タブ順序] タブを並べ替え中...');
        const savedUnpinned = savedData.tabs.filter(t => !t.pinned);
        const usedTabIds = new Set();

        for (let i = 0; i < savedUnpinned.length; i++) {
          const savedTab = savedUnpinned[i];
          const candidates = tabsByUrl.get(savedTab.url) || [];
          const matchingTab = candidates.find(t => !usedTabIds.has(t.id));
          ...
          if (matchingTab) {
            ...
            try {
              // タブが存在するか確認してから移動
              const tabCheck = await chrome.tabs.get(matchingTab.id).catch(() => null);
              if (tabCheck) {
        ...
        // ステップ4: グループを作成（最新のタブ状態を再取得）
        console.log('[タブ順序] グループを作成中...');
        currentTabs = await chrome.tabs.query({ windowId: win.id });
        const newGroupMap = new Map();
        const groupOrder = savedData.groups || [];
        
        // URLでタブを再マッピング
        const currentTabsByUrl = new Map();
        for (const tab of currentTabs) {
          if (!tab.pinned) {
            if (!currentTabsByUrl.has(tab.url)) {
              currentTabsByUrl.set(tab.url, []);
            }
            currentTabsByUrl.get(tab.url).push(tab);
          }
        }

        for (const groupInfo of groupOrder) {
          if (!groupInfo.title) continue;

          // このグループに属するべきタブを探す
          const groupTabIds = [];
          const usedUrlIndices = new Map();

          for (const savedTab of savedUnpinned) {
            if (savedTab.groupInfo && savedTab.groupInfo.title === groupInfo.title) {
              const urlTabs = currentTabsByUrl.get(savedTab.url) || [];
              const urlIndex = usedUrlIndices.get(savedTab.url) || 0;
              
              if (urlIndex < urlTabs.length) {
                const currentTab = urlTabs[urlIndex];
                if (currentTab && !groupTabIds.includes(currentTab.id)) {
                  groupTabIds.push(currentTab.id);
                }
                usedUrlIndices.set(savedTab.url, urlIndex + 1);
              }
            }
          }
          ...
          if (groupTabIds.length > 0) {
            try {
              ...
              if (validTabIds.length > 0) {
                ...
                newGroupMap.set(groupInfo.title, newGroupId);
                console.log('[タブ順序] グループ作成:', groupInfo.title, '(', validTabIds.length, 'タブ)');
              }
            } catch (e) {
              console.error('[タブ順序] グループ作成エラー:', groupInfo.title, e.message);
            }
          }
        }
        ...
        // ステップ5: 折りたたみ状態を復元
        await new Promise(resolve => setTimeout(resolve, 300));
        for (const [title, groupId] of newGroupMap) {
          const savedGroup = groupOrder.find(g => g.title === title);
          if (savedGroup && savedGroup.collapsed) {
            try {
  ...
  chrome.tabs.onAttached.addListener(debouncedSave);
  chrome.tabs.onDetached.addListener(debouncedSave);
  ...
  if (chrome.tabGroups) {
    ...
    chrome.tabGroups.onUpdated.addListener(debouncedSave);
  }

  // ウィンドウ閉じる前に保存
  chrome.windows.onRemoved.addListener(saveTabOrderImmediately);

  // メッセージリスナー
  chrome.runtime.onMessage.addListener(function(msg, sender, sendResponse) {
    if (msg.restoreTabOrder) {
      restoreTabOrder().then(function() { sendResponse({ success: true }); });
      return true;
    }
    if (msg.saveTabOrder) {
      saveTabOrderImmediately().then(function() { sendResponse({ success: true }); });
      return true;
    }
  });
  ...
  async function reloadAllTabs() {
    console.log('[タブ順序] すべてのタブを再読み込み中...');
    try {
      const tabs = await chrome.tabs.query({});
      const reloadPromises = [];

      for (const tab of tabs) {
        // chrome://やedge://などの特殊ページはスキップ
        if (tab.url && !tab.url.startsWith('chrome://') && !tab.url.startsWith('edge://') && !tab.url.startsWith('chrome-extension://')) {
          reloadPromises.push(
            chrome.tabs.reload(tab.id).catch(e => {
              console.log('[タブ順序] リロードスキップ:', tab.url);
            })
          );
        }
      }
      ...
      console.log('[タブ順序] 再読み込み完了:', tabs.length, 'タブ');
      return true;
    } catch (e) {
      console.error('[タブ順序] 再読み込みエラー:', e);
      return false;
    }
  }

  // すべてのタブの読み込み完了を待つ
  async function waitForTabsLoaded(timeout = 10000) {
    console.log('[タブ順序] タブの読み込み完了を待機中...');
    const startTime = Date.now();

    while (Date.now() - startTime < timeout) {
      const tabs = await chrome.tabs.query({});
      const loadingTabs = tabs.filter(t => t.status === 'loading');

      if (loadingTabs.length === 0) {
        console.log('[タブ順序] すべてのタブの読み込み完了');
        return true;
      }
    ...
    console.log('[タブ順序] タイムアウト - 読み込み待機終了');
    return false;
  }

  // 起動時の処理: 待機 → 復元 (v2.4)
  async function handleStartup() {
    console.log('[タブ順序] ブラウザ起動を検出 (v2.4)');
    isRestoring = true;

    try {
      // Chromeの初期化と元のAcid Tabs処理が完了するまで待つ
      console.log('[タブ順序] 初期化待機中... (5秒)');
      await new Promise(resolve => setTimeout(resolve, 5000));

      // タブの読み込み完了を待つ
      await waitForTabsLoaded(10000);

      // 元のAcid Tabsのグループ化処理が完了するまで追加で待機
      console.log('[タブ順序] グループ化処理の完了を待機中... (3秒)');
      ...
      console.log('[タブ順序] 起動時の復元処理完了');
    } catch (e) {
      console.error('[タブ順序] 起動時処理エラー:', e);
      isRestoring = false;
    }
  }
  ...
  chrome.runtime.onInstalled.addListener(function(details) {
    if (details.reason === 'update') {
      console.log('[タブ順序] 拡張機能更新を検出');
      handleStartup();
    }
  });

  // 定期保存（5分ごと）
  setInterval(saveTabOrder, 5 * 60 * 1000);

  // 初期保存
  setTimeout(saveTabOrder, 2000);

  console.log('[タブ順序] 管理機能 v2.4 を初期化しました');
})();

...
(()=>{
  ...
  async function ungroupAllTabs(windowId) {
    ...
    try {
      ...
      const groupedTabs = tabs.filter(t => t.groupId !== -1 && !t.pinned);
      ...
      console.log('[タブ順序] グループ解除完了:', groupedTabs.length, 'タブ');
    } catch (e) {
      console.error('[タブ順序] グループ解除エラー:', e);
    }
  }

  // タブ順序を復元 (v2.2 - 完全再構築方式)
  async function restoreTabOrder() {
    isRestoring = true;
    console.log('[タブ順序] 復元処理開始 (v2.2)');
    ...
    try {
      ...
      console.log('[タブ順序] 保存データ:', savedData.tabs.length, 'タブ');
      ...
      for (const win of windows) {
        ...
        for (let i = 0; i < savedUnpinned.length; i++) {
          ...
          if (matchingTab) {
            ...
            try {
              await chrome.tabs.move(matchingTab.id, { index: pinnedCount + i });
            } catch (e) {}
          }
        }
        ...
        for (const groupInfo of groupOrder) {
          ...
          for (const savedTab of savedUnpinned) {
            if (savedTab.groupInfo && savedTab.groupInfo.title === groupInfo.title) {
              const currentTab = currentTabs.find(t => t.url === savedTab.url && !t.pinned);
              if (currentTab && !groupTabs.some(gt => gt.id === currentTab.id)) {
                groupTabs.push(currentTab);
              }
            }
          }
          ...
          if (groupTabs.length > 0) {
            try {
              ...
              console.log('[タブ順序] グループ作成:', groupInfo.title);
            } catch (e) {
              console.error('[タブ順序] グループ作成エラー:', e);
            }
          }
        }
  ...
  async function handleStartup() {
    console.log('[タブ順序] ブラウザ起動を検出 (v2.2)');
    isRestoring = true;
  ...
  console.log('[タブ順序] 管理機能 v2.2 を初期化しました');
})();
